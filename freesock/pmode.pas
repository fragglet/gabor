{
    $Id: pmode.pas,v 1.0 1999/07/07 09:46:55 gabor Exp $
...
type
    ...
    {$endif}
      ...
      procedure MoveDataToOfs(AOfs: word; const Src; DSize: word);
      procedure MoveDataFromOfs(AOfs: word; DSize: word; var Dest);
      procedure Clear;
    private
...
const HaltOnDosAllocFail: boolean = true;
...
{$else}
  ...
  M.Sel:=(L and $ffff);
{$endif}
  if (M.Seg=0) and (Size>0) and HaltOnDosAllocFail then
    RunError(203);
...
begin
  MoveDataToOfs(0,Src,DSize);
end;

procedure MemPtr.MoveDataToOfs(AOfs: word; const Src; DSize: word);
begin
  if AOfs+DSize>Size then
    RunError(216);
  Move(Src,Ptr(DataSeg,DataOfs+AOfs)^,DSize);
end;

procedure MemPtr.MoveDataFrom(DSize: word; var Dest);
begin
  MoveDataFromOfs(0,DSize,Dest);
end;

procedure MemPtr.MoveDataFromOfs(AOfs: word; DSize: word; var Dest);
begin
  if AOfs+DSize>Size then
    RunError(216);
  Move(Ptr(DataSeg,DataOfs+AOfs)^,Dest,DSize);
end;
